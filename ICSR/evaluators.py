...
def extract_features_per_cam(model, data_loader, print_freq):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, camid, _) in enumerate(data_loader):
            ...
            for fname, output, pid, cam in zip(fnames, outputs, pids, camid):
                ...
                y1 .append(cam.cpu().numpy())
                y2.append(pid.cpu().numpy())
                cam = cam.item()
                per_cam_features[cam][fname] = output
                per_cam_label[cam].append(pid.cpu().numpy().tolist())
            ...
            if (i + 1) % print_freq == 0:
                    print('Extract Features: [{}/{}]\t'
                        ...
                        .format(i + 1, len(data_loader),
                                batch_time.val, batch_time.avg,
                                data_time.val, data_time.avg))
    visualization(np.array(x),np.array(y1),np.array(y2))
    return per_cam_features, per_cam_label
